...
def test_post_adv(create_user, title = 'test_title', descr = 'test_descr'):
    params = {'title': title, 'descr': descr, 'user_id': create_user['id'], 'password': 'password'}
    response = requests.post(f'{API_URL}/adv/', json=params)
    assert response.status_code == 200


def test_get_adv():
    response = requests.get(f'http://127.0.0.1:5000/adv/1/')
    assert response.json()['id'] == 1


def test_patch_adv(adv_id = 1, title = 'alt_title', descr = 'alt_descr', user_id = 2, password = 'password'):
    params = {'title': title, 'descr': descr, 'user_id': user_id, 'password': password}
    response = requests.patch(f'{API_URL}/adv/{adv_id}/', json=params)
    assert response.status_code == 200


def test_patch_adv_wrong_user(adv_id = 1, title = 'alt_title', descr = 'alt_descr', user_id = 1, password = 'password'):
    params = {'title': title, 'descr': descr, 'user_id': user_id, 'password': password}
    response = requests.patch(f'{API_URL}/adv/{adv_id}/', json=params)
